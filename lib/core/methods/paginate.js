...
module.exports = function(env) {
	...
	return function(args) {
		...
		// default the args object to an empty object
		args = !u.isHash(args) ? { request: {} } : args;
		args.request = !u.isHash(args.request) ? {} : args.request;
		
		_self._pagination_var      = _.has(_self, '_pagination_var') ? _self._pagination_var : {};
		_self._pagination_var.path = args.path || _self.tableName;
		...
		_self._pagination_var.results = u.resolveInput(null, _self).then(function(results) {
			...
			if (typeof(args.request) === _JTYP.object) {
				...
				// allow users to specify the query field but default to req.query or req.params
				var p = _.get(req, args.queryField) || req.query || req.params;
				
				// TODO: deprecate the useQuery option and use the queryField option instead
				p     = (args.useQuery === true && req.query) ? req.query : p;
				...
				var getPagination = function(t) {
					...
					.then(function() {
						...
						return _self.query(function(qb) {
							...
							qb = limit !== -1 ? qb.limit(limit) 		 : qb;
							qb = offset       ? qb.offset(offset)            : qb;
						})
						.fetchAll(_.omit(args, ['request', 'path', 'useQuery', 'queryField']))
						.then(function(results) {
							...
							if (results) {
								...
								results = results.toJSON(_.omit(args, [
									'request', 'path', 'useQuery',
									'queryField', 'transacting', 'withRelated'
								]));
