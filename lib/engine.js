	...
	function makePage(args) {
		...
		.then(function(columns) {
			...
			return m1.query(function(qb) {
				if (orderSQL !== '') {
					qb.limit(length).offset(start).orderByRaw(orderSQL).whereRaw(where + filterSQL);
				}
				else {
					qb.limit(length).offset(start).whereRaw(where + filterSQL);
				}
			})
		...
		.then(function() {
			...
			return util.getTotalFilteredRecords(m2, where + filterSQL).then(function(count) {
